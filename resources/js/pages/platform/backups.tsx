    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                ...
                <DialogDescription>
                    {backup.filename} will be permanently removed from the
            ...
            <PageShell>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <HardDriveDownload className="size-4" /> Full
                                site backup
                            </CardTitle>
                            <CardDescription>
                                Every application table (schema + data) plus
                                files under <code>storage/app/public</code>.
                                Session, cache, queue and migration tables are
                                excluded.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Database className="size-4" /> Single workspace
                                backup
                            </CardTitle>
                            <CardDescription>
                                Only the selected workspace's rows across all
                                tenant tables. Uploaded files are not
                                tenant-mapped, so they are excluded here.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Form
                                action={
                                    tenantId
                                        ? `/platform/backups/tenant/${tenantId}`
                                        : '#'
                                }
                                method="post"
                                className="space-y-3"
                ...
                <Card className="bg-muted/20">
                    <CardContent className="pt-6 text-sm">
                        <p className="font-medium">
                            Restoring is a manual, server-side step.
                        </p>
                        <p className="text-muted-foreground mt-1 leading-6">
                            This platform runs all tenants in one shared
                            database, so restoring from the browser could
                            overwrite or clash with other tenants' rows.
                            Download the archive and import it deliberately:
                        </p>
